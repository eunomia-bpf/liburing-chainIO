...
static int recv_ts_iou(int fd)
{
	...
	ret = io_uring_queue_init(8, &ring, IORING_SETUP_CQE32 | IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_DEFER_TASKRUN);
	if (ret)
		error(1, ret, "queue init\n");
	...
	while (1) {
		...
		fprintf(stderr, "CQE | ud %li res %i flag %i\n",
			(long)cqe->user_data, cqe->res, cqe->flags);
		fprintf(stderr, "\t sec %lu, nsec %lu\n", (long)cqe->big_cqe[0],
							  (long)cqe->big_cqe[1]);

		io_uring_cqe_seen(&ring, cqe);
		if (!(cqe->flags & IORING_CQE_F_MORE)) {
			fprintf(stderr, "last cqe\n");
			break;
		}
	}
...
static void resolve_hostname(const char *hostname)
{
	...
	for (cur = addrs; cur; cur = cur->ai_next) {
		if (cur->ai_family == AF_INET6)
			break;
	}

	if (!cur)
		error(1, 0, "can't resolve hostname");
