...
static int test_send_tag_override(struct io_uring *ring, int sock_tx, int sock_rx)
{
	unsigned zc_flags = IORING_SENDZC_FLUSH | IORING_SENDZC_OVERRIDE_TAG;
	int msg_flags = 0;
	int payload_size = 1;
	int ret, j;

	for (j = 0; j < NR_SLOTS * 3; j++) {
		struct io_uring_sqe *sqe;
		struct io_uring_cqe *cqe;
		int slot_idx = rand() % NR_SLOTS;
...
static int test_multi_send_flushing(struct io_uring *ring, int sock_tx, int sock_rx)
{
	struct io_uring_sqe *sqe;
	struct io_uring_cqe *cqe;
	unsigned zc_flags = IORING_SENDZC_FLUSH;
			     ...
			     size_t send_size, bool cork, bool mix_register)
{
	...
	assert(send_size <= sizeof(tx_buffer));
...
int main(int argc, char *argv[])
{
	...
	ret = test_send_tag_override(&ring, sp[0], sp[1]);
	if (ret) {
		fprintf(stderr, "test_send_tag_override() failed\n");
		return ret;
	}
