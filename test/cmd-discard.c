...
#define BLOCK_URING_CMD_WRITE_ZERO_PAGE		2
#endif
...
static int cmd_issue_verfiy(struct io_uring *ring, int fd, int lba, int len,
			    int cmd_op)
{
	...
	if (ret) {
		...
		fprintf(stderr, "cmd_issue_verfiy %i fail lba %i len %i  ret %i\n",
				cmd_op, lba, len, ret);
		return T_EXIT_FAIL;
	}
...
static int basic_cmd_test(struct io_uring *ring, int cmd_op)
{
	...
	ret = cmd_issue_verfiy(ring, fd, 0, 1, cmd_op);
	if (ret == T_EXIT_SKIP) {
	...
	ret = cmd_issue_verfiy(ring, fd, 7, 15, cmd_op);
	if (ret) {
		fprintf(stderr, "cmd %i fail 7 15 %i\n", cmd_op, ret);
		return T_EXIT_FAIL;
	}

	ret = cmd_issue_verfiy(ring, fd, 1, MAX_TEST_LBAS - 1, cmd_op);
	if (ret) {
